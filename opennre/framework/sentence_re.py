...
class SentenceRE(nn.Module):
    ...
    def __init__(self, 
        ...
        if opt == 'sgd':
            ...
            self.optimizer = optim.Adagrad(params, lr, weight_decay=weight_decay)
        elif opt == 'adadelta':
    ...
    def eval_model(self, eval_loader, metric='default'):
        ...
        if metric == 'default':
            ...
            result.update(self.F1_score_macro(
                torch.cat(preds, dim=0), torch.cat(labels, dim=0), logits.shape[1]), dataset='other')
    ...
    def F1_score_macro(self, pred, label, class_num, threshold=0.5, beta=1, dataset='semeval'):
        ...
        if dataset == 'semeval':
            ...
            TP = (prob & label).sum(dim=0).float()
            TN = ((~prob) & (~label)).sum(dim=0).float()
            FP = (prob & (~label)).sum(dim=0).float()
            FN = ((~prob) & label).sum(dim=0).float()
