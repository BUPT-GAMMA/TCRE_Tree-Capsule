...
class BaseEncoder(nn.Module):
    ...
    def __init__(self, 
        ...
        if pos_ner:
            self.input_size += position_size * 2
        ...
        if word2vec is not None:
            ...
            print('Finished!')
        self.word2vec = word2vec   # 这是numpy
        ...
        self.pos_embedding = nn.Embedding(60, self.position_size, padding_idx=0)
        self.ner_embedding = nn.Embedding(20, self.position_size, padding_idx=0)
    ...
    def tokenize(self, item):
        ...
        if self.blank_padding:                
            ...
            if 'pos' in item:
                ...
                ner = ner[:self.max_length]

        indexed_tokens = torch.tensor(indexed_tokens).long().unsqueeze(0) # (1, L)
        pos1 = torch.tensor(pos1).long().unsqueeze(0) # (1, L)
        pos2 = torch.tensor(pos2).long().unsqueeze(0) # (1, L)
        if 'pos' in item:
            pos = torch.tensor(pos).long().unsqueeze(0) # (1, L)
            ner = torch.tensor(ner).long().unsqueeze(0) # (1, L)
        else:
            pos, ner = None, None

        # if USETREE:
        return indexed_tokens, pos1, pos2, tree, pos, ner
        # else:
